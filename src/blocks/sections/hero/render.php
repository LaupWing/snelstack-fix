...
$heading    = $attributes['heading']           ?? 'Websites die werken.';
$cta_1_label = $attributes['ctaPrimaryLabel']  ?? 'Start je project';
$cta_1_url   = $attributes['ctaPrimaryUrl']    ?? '#contact';
$cta_2_label = $attributes['ctaSecondaryLabel'] ?? 'Bekijk ons werk';
$cta_2_url   = $attributes['ctaSecondaryUrl']   ?? '#werk';
...
<section class="relative">
    <?php snel_background_open(['position' => 'absolute']); ?>
        <div class="px-4 pt-40 pb-20 md:px-8 lg:pt-44">
            <?php snel_panel_open(['inner_class' => 'gap-8 xl:gap-12']); ?>
                ...
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <a href="<?php echo esc_url($cta_2_url); ?>" class="group inline-flex h-12 items-center gap-2 rounded-full border-2 border-violet-400/40 bg-white px-5 font-medium text-violet-600 transition-all duration-300 hover:bg-violet-600 hover:text-white">
                        <?php echo $arrow; ?>
                        <span class="whitespace-nowrap"><?php echo esc_html($cta_2_label); ?></span>
                    </a>
                </div>
